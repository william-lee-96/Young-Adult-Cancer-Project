...
################################ FISHER'S EXACT AMPEVNT ANALYSIS ################################

fisher_ampEvnt_df <- data.frame(matrix(ncol = 3, nrow = 0))
colnames(fisher_ampEvnt_df) <- c("cancer", "ampEvnt", "p_value")

for (cancer in unique(clin_amplification_therapeutics_ggplot_rmNone$acronym)) {
  
  current_cancer_df = clin_amplification_therapeutics_ggplot_rmNone[clin_amplification_therapeutics_ggplot_rmNone$acronym==cancer,]
  
  for (ampEvnt in unique(current_cancer_df$ampEvnt)) {
    ...
    num_not_current_ampEvnt_ltr = sum(current_cancer_df$age_binary==FALSE & current_cancer_df$ampEvnt!=ampEvnt)
    num_not_current_ampEvnt_yng = sum(current_cancer_df$age_binary==TRUE & current_cancer_df$ampEvnt!=ampEvnt)
    
    yng_set = c(num_not_current_ampEvnt_yng, num_current_ampEvnt_yng)
    late_set = c(num_not_current_ampEvnt_ltr, num_current_ampEvnt_ltr)
    
    ampEvnt_fisher = cbind(yng_set, late_set)
    
    ampEvnt_fisher_pval = fisher.test(ampEvnt_fisher)$p.val
    
    temp = cbind(cancer, ampEvnt, ampEvnt_fisher_pval)
    colnames(temp) <- c("cancer", "ampEvnt", "p_value")
    
    fisher_ampEvnt_df = rbind(fisher_ampEvnt_df, temp)
    
  }
}

fisher_ampEvnt_df$p_value = as.numeric(as.character(fisher_ampEvnt_df$p_value))
fisher_ampEvnt_df$FDR = p.adjust(fisher_ampEvnt_df[,"p_value"], method="fdr") 
fisher_ampEvnt_df=fisher_ampEvnt_df[order(fisher_ampEvnt_df$p_value, decreasing=FALSE),]
