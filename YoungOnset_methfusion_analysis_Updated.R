...
for (cancer in unique(clin_merge_subtype_avail_complete$acronym)){
  ...
  for (mf in alt_events){
    ...
    mf_df_current = select(cnv_df, bcr_patient_barcode, mf) # change line to specify which type you are looking at (meth, fusion, mut, cnv) 
...
tn = "~/Box/Huang_lab/manuscripts/YoungOnsetCancer/analysis/methylation_fusion/out/cnv_events_onsetAge_assoc_Updated.txt"
...
tt$FDR = p.adjust(tt[,"P_Chi"], method="fdr") 
tt=tt[order(tt$P_Chi, decreasing=FALSE),]
tn = "~/Box/Huang_lab/manuscripts/YoungOnsetCancer/analysis/methylation_fusion/out/cnv_events_onsetAgeBinary_assoc_Updated.txt"
write.table(tt, quote=F, sep="\t", file = tn, row.names = F)